...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "biscuiteer", "tests")); system.time(test_check("biscuiteer", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
