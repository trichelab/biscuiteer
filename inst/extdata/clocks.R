...
horvath <- read.csv("HorvathClock.csv", row.names=1)[, 1:2]
coefs <- list(
  ...
  skinandblood=read.csv("SkinAndBloodClock.csv", row.names=1)[, 1, drop=FALSE],
  hannum=read.csv("HannumClock.csv", row.names=1)[, 5, drop=FALSE]
)
...
if (reload) {
  ...
  mfests <- readRDS("mfests.rds") # subsetted (much much smaller)
  regBuilds <- readRDS("regBuilds.rds") 
}
